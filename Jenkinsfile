...
@Library('mdop-pluggable-scm-jenkinsfile') _

def repoName = "spring-petclinic"
def regRepo = "mdop-cartridge-java-regression-tests"

pipeline {
    agent { label 'java8' }
    environment{
        // Sonar Project Info - You must replace these vars with the Project Name and Key from the MDOP Portal!
        ...
        stage("Reference Application Code Analysis"){
            steps{
                ...
                script{
                    ...
                    scannerHome = tool 'MDOP SonarScanner'
                    withSonarQubeEnv('MDOP Sonar') {
